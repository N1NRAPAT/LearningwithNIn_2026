...
int main(void)
{
    ...
    do
    {
        Arm_setup(&robot_arm); /* setup arm lengths at the start of every run */
...
void Arm_setup(struct Arm *arm)
{
    ...
    if (arm->min_reach < 0)
        arm->min_reach = -arm->min_reach; /* ensure min_reach is always positive */
...
float validate_float(const char *prompt, float min, float max)
{
    ...
    do
    {
        ...
        if (result != 1)
        { /* scanf failed — user typed letters or symbols */
...
void get_target_position(struct Target *target, struct Arm arm)
{
    ...
    do
    {
        ...
        if (y_min > 0.0f && (target->y < 0.0f ? -target->y : target->y) < y_min)
        {
...
void Calculate_Joint_Angles(struct Arm *arm, struct Target target, float *t_angle1, float *t_angle2)
{
    ...
    if (cos_angle2 > 1.0f)
        cos_angle2 = 1.0f; /* clamp upper bound */
    if (cos_angle2 < -1.0f)
        cos_angle2 = -1.0f; /* clamp lower bound */
...
void animate_movement(struct Arm *arm, float target_angle1, float target_angle2, struct Target target)
{
    ...
    for (step = 0; step <= num_steps; step++)
    {
        ...
        /* Round tiny near-zero values to 0.0 so they display as 0.00, not -0.00 */
        float tip_x = (arm->joints[1].x > -0.005f && arm->joints[1].x < 0.005f) ? 0.0f : arm->joints[1].x;
        float tip_y = (arm->joints[1].y > -0.005f && arm->joints[1].y < 0.005f) ? 0.0f : arm->joints[1].y;
        ...
        printf("%-6d %-12.2f %-12.2f %-12.2f %-12.2f %.3f cm\n",
               ...
               tip_x,                                /* tip x at this step */
               tip_y,                                /* tip y at this step */
    ...
    float final_x = (arm->joints[1].x > -0.005f && arm->joints[1].x < 0.005f) ? 0.0f : arm->joints[1].x;
    float final_y = (arm->joints[1].y > -0.005f && arm->joints[1].y < 0.005f) ? 0.0f : arm->joints[1].y;
